                    ...
                    if text.upper() == "CONFIRM":
                        ...
                        if row:
                            ...
                            await conn.execute("""
                                ...
                                VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'pending', $9) ON CONFLICT (order_hash) DO NOTHING
                        ...
                        if res.get('final_order_data'):
                            # Database စျေးနှုန်းဖြင့်သာ တွက်ချက်ခြင်း (လုံခြုံရေးအတွက်)
                            ...
                            await conn.execute("""
                                INSERT INTO pending_orders (chat_id, business_id, order_data, updated_at) VALUES ($1, $2, $3, NOW())
                            ...
                            if res.get('intent') == "confirm_order" and valid_items:
                                ...
                                for item in valid_items: summary += f"- {item['name']} x {item['qty']} ({item['price'] * item['qty']} MMK)\n"
                                summary += f"\n💰 *စုစုပေါင်း: {total} MMK*\n\n"
                                if p_method == "Pre-paid": summary += "⚠️ *ငွေလွှဲပြေစာကို Admin ထံ ပို့ပေးပါရန်။*\n\n"
